...
def create_jwt_for_user(user) -> dict:
    """
    Genera tokens JWT (access + refresh) para un usuario dado.
    Usa SimpleJWT y devuelve ambos tokens en formato string.
    """
    try:
        ...
        logger.error(f"❌ Error al generar tokens JWT para usuario {user.pk}: {e}")
        raise
